...
public class Canvas extends JComponent {
    ...
    private Modes.ObjectModes objectMode = Modes.ObjectModes.VERTEX; //Текущий режим (вершины или линии)
    private Modes.OperationModes operationModes = Modes.OperationModes.ADD;
    ...
    public void changeObjectMode(Modes.ObjectModes newMode) {
        objectMode = newMode;
    }

    public void changeOperationMode(Modes.OperationModes newMode) {
        operationModes = newMode;
    }
    ...
    private void handleClick(Point clickPoint) {
        ...
        if (operationModes == Modes.OperationModes.ADD)
    ...
    private void handleAddModeClick(Point clickPoint) {
        ...
        if (objectMode == Modes.ObjectModes.VERTEX) { //Режим вершин
    ...
    private void drawVertex(Vertex v) {
        ...
        int drawingCoordinatesOfX = v.getPos().x - halfSize;
        int drawingCoordinatesOfY = v.getPos().y - halfSize;

  //      g.setStroke(new BasicStroke(4));

        g.setColor(Color.WHITE);
        g.fillOval(drawingCoordinatesOfX, drawingCoordinatesOfY, vertexSize, vertexSize);
        g.setColor(v.getColor());
        g.drawOval(drawingCoordinatesOfX, drawingCoordinatesOfY, vertexSize, vertexSize);

        String strNumOfVert = String.valueOf(v.getId());
        Rectangle2D stringBounds = g.getFontMetrics(g.getFont()).getStringBounds(strNumOfVert, g);

        g.setColor(Color.BLACK);
        g.drawString(strNumOfVert, v.getPos().x - (int) stringBounds.getCenterX(), v.getPos().y - (int) stringBounds.getCenterY());
    }
    ...
    private void drawResistor(Resistor resistor) {
        ...
        double theta = Math.atan2(resistor.getEnd().getPos().y - resistor.getStart().getPos().y, resistor.getEnd().getPos().x - resistor.getStart().getPos().x);
