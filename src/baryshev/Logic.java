...
public class Logic {
     ...
     *
     * @param checkingVertex Вершина для проверки
     * @param graph    Граф, в котором будет осуществлена проверка
     */
    private static boolean isParallelStart(Integer checkingVertex, Graph graph) {
        Iterable<Integer> adj = graph.adjacency(checkingVertex);
    ...
    private static AdjMatrixWeightedDigraph graphOrientating(WeightedGraph graph, int startNode, int endNode) {
...
//                parallelResistance = serialSum != 0 ? 1 / (serialSum) : 0;
    ...
    private static int searchForEndOfSerial(boolean[][] adjMatrix, int curr) {
        ...
        if (countOfInput > 1 || countOfOutput > 1) {
            ...
            else return searchForEndOfSerial(adjMatrix, next);
        } else //Если же счетчик остался равен нулю, то это конец графа, и текущая вершина - конец соединения
            return curr;
    }

    private static boolean checkDidWeHavePath(WeightedGraph inputGraph, int startNode, int endNode, boolean[] visited) {
        ...
        for (int v : inputGraph.adjacency(startNode)) {
            if (v == endNode) {
                havePath = true;
            }
            else if (!visited[v]) {
                if (checkDidWeHavePath(inputGraph, v, endNode, visited)) {
                    havePath = true;
                }
            }
        }
    ...
    public static double calcCircuitResistance(WeightedGraph inputGraph, int startNode, int endNode) {
        if (!checkDidWeHavePath(inputGraph, startNode, endNode, new boolean[inputGraph.vertexCount()])) {
            return -1;
        }
        AdjMatrixWeightedDigraph digraph = graphOrientating(inputGraph, startNode, endNode);
